...
let make_threshold_manager
  ...
  { manager with
    ...
    is_bottom = begin fun _ set -> PSette.is_empty set end;
    is_leq = begin fun _ set1 set2 -> PSette.subset set1 set2 end;
    join = begin fun _ set1 set2 -> PSette.union set1 set2 end;
    join_list =
      begin fun _ lset ->
	List.fold_left
	  PSette.union
    ...
    widening = begin fun _ _ _ -> failwith "" end;
    odiff = Some(begin fun _ set1 set2 -> PSette.diff set1 set2 end);
    abstract_init = parameter.init;
    FixpointType.arc_init = begin fun _ -> () end;
