...
module ExprProp(R:Num)(X:X) = struct
  ...
  | Expr(op,es) ->
      ...
      let sum_cs = fold_left (function_of_op op) (neutral_element_of_op op) (map (function x -> match x with
      Const n -> n
      | _ -> assert false) cs) in 
      match es with
        [] -> Const sum_cs
  ...
  let atom_linearize (e1,rel_sym,e2) = 
    ...
    let (c1, e1) = match e1 with 
    |Expr(Add, c :: e) -> (c,e)
    | _ -> assert false in
    let (c2, e2) = match e2 with
    | Expr(Add, c :: e) -> (c,e)
    | _ -> assert false in
  ...
  let pform_partial_eval = pform_simplify_atoms atom_partial_eval
  
  let pform_linearize = pform_simplify_atoms atom_linearize
