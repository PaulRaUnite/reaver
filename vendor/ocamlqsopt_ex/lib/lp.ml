...
module Make(X : X) = struct
  ...
  | Sum(es)  -> List.fold_left vars_of_expr l es
  ...
  | Sum(es)  -> List.exists contains_var es
  
  let rec string_of_expr = function 
    Const n  -> to_string n
  | Var x    -> string_of_x x
  | Mul(c,e) -> to_string c ^ " * " ^ string_of_expr e
  | Sum(es)  -> "(" ^ String.concat " + " (List.map string_of_expr es) ^ ")"
  ...
  let string_of_constrs constrs = 
    String.concat "\n" (List.map string_of_constr constrs)
  ...
  | Sum(es)  -> List.fold_left (normal_expr factor) (const,lin) es
  let normal_expr = normal_expr one (zero,[])
  let normal_expr e =
    let (c,l) = normal_expr e in
    let l = List.sort (fun (_,x) (_,y) -> compare x y) l in
    ...
    in
    let l = doit l in
    let l = List.filter (fun (c,_) -> not (c =. zero)) l in
    (c,l)
  ...
  let string_of_normal_constr = function
    NEq(lhs,b) -> String.concat " + " (List.map (fun (c,x) -> to_string c ^ " * " ^ string_of_x x) lhs) ^ " = " ^ to_string b
  | NLe(lhs,b) -> String.concat " + " (List.map (fun (c,x) -> to_string c ^ " * " ^ string_of_x x) lhs) ^ " <= " ^ to_string b
  ...
  let is_bound = function NEq(lhs,_) | NLe(lhs,_) -> List.length lhs = 1
  ...
  let solve lp =
    ...
    let init_variables_nexpr ne = 
      ...
      List.iter (fun x -> let _ = get_var_id x in ()) vars
    in
    ...
    *)
    let obj = match lp.obj with Max obj -> obj | _ -> assert false in
    let (add,c) = normal_expr obj in
    let add = InfGmpRatio.Num add in
    let constrs = List.map normal_constr lp.constrs in
    if List.exists is_trivially_unsat constrs then
      (InfGmpRatio.Neginfty, fun _ -> raise Unsup)
    else
      let constrs = List.filter is_non_trivial constrs in
      assert(List.for_all (function NEq(_::_,_) | NLe(_::_,_) -> true | _ -> false) constrs);
      (* let _ = init_variables_nexpr c in *)
      let _ = List.iter init_variables_nconstr constrs in
      (* Die naechste Zeile ist nur mal fuer den Moment, sonst probleme *)
      let is_bound _ = false in
      let bounds, constraints = List.partition is_bound constrs in
      let number_of_variables = !next_id in 
      let number_of_constraints = List.length constraints in
      let number_of_values = 
        List.fold_left 
          ( + ) 
          0 
          (List.map (function NEq(lhs,_) | NLe(lhs,_) -> List.length lhs) constraints) 
      in
      let is_unconstrained x =
        ...
        | _ -> assert false
      in
      if List.exists is_unconstrained (let _,vars = split c in vars) then
        ...
        begin
          if contains_var obj then
            ...
            | Var _ -> raise Unsup
            | Mul(c,e) -> c *. (eval e)
            | Sum(es) -> List.fold_left ( +. ) zero (List.map eval es)
            in
        ...
        begin
          ...
          let cmat = Array.make number_of_variables [] in
          List.iteri 
            (
            ...
            constraints;
          Array.iteri (fun i l -> cmat.(i) <- List.rev l) cmat;
          let cmatcnt = Array.map List.length cmat in
          Array.iteri (set_cmatcnt lp) cmatcnt;
          let cmatbeg = Array.fold_left (function acc -> match acc with (b::_) as bs -> fun l -> (b + l) :: bs | _ -> assert false) [0] cmatcnt in
          let cmatbeg = List.tl cmatbeg in
          let cmatbeg = List.rev cmatbeg in
          List.iteri (set_cmatbeg lp) cmatbeg;
          let (cmatind,cmatval) = split (List.flatten (Array.to_list cmat)) in
          List.iteri (set_cmatind lp) cmatind;
          List.iteri 
            (set_sense lp) 
            (List.map (function NEq _ -> Comp_eq | NLe _ -> Comp_le) constraints);
          List.iteri (set_cmatval lp) cmatval;
          let obj = Array.make number_of_variables zero in
          List.iteri (fun _ (c,x) -> let id = get_var_id x in obj.(id) <- obj.(id) +. c) c; 
          Array.iteri (set_obj lp) obj;
          List.iteri (set_rhs lp) (List.map (function NEq(_,b) | NLe(_,b) -> b) constraints);
 ...
 (* ------------- *)
  ...
  let solve' prob_name constrs =
    ...
    in
    let init_variables_nconstr = function NEq(ne,_) | NLe(ne,_) -> init_variables_nexpr ne in
    let constrs = List.map normal_constr constrs in
    if List.exists is_trivially_unsat constrs then
      fun _ -> (InfGmpRatio.Neginfty, fun _ -> raise Unsup)
    else
      let constrs = List.filter is_non_trivial constrs in
      assert(List.for_all (function NEq(_::_,_) | NLe(_::_,_) -> true | _ -> false) constrs);
      let _ = List.iter init_variables_nconstr constrs in
      ...
      fun obj -> 
        ...
        let obj = match obj with Max obj -> obj | _ -> assert false in
        let (add,c) = normal_expr obj in
        let add = InfGmpRatio.Num add in
        let is_unconstrained x =
          match Hashtbl.find_all ht x with
...
module Dualizer(PrimalLP : LP) = struct
  ...
  let dualize_obj_expr constrs =
    DualLP.Sum(List.map dualize_for_constr constrs)
  
  let create_dual_obj constrs obj = 
    match obj with
      PrimalLP.Max _ -> DualLP.Min (dualize_obj_expr constrs)
    | PrimalLP.Min _ -> DualLP.Max (dualize_obj_expr constrs)
  ...
  | x::l -> 
      begin
        ...
        | _::_                      -> partition compare (part::acc) [x] l
      end 
  ...
  let create_dual_constrs constrs (_,lin_obj) =
    let handle_constr (constr_acc, slack_acc) pconstr = 
      let (constr,nconstr) = pconstr in
      match nconstr with
        PrimalLP.NLe(lhs,_) ->
          (((List.rev_map (fun (c,x) -> (x,pconstr,c)) lhs)::constr_acc), ((DualLP.Le(DualLP.Const(from_string "0"), DualLP.Var(constr)))::slack_acc ) )
      | PrimalLP.NEq(lhs,_) -> 
          (((List.rev_map (fun (c,x) -> (x,pconstr,c)) lhs)::constr_acc), slack_acc)
    in
    let (lconstrs, slacks) = List.fold_left handle_constr ([],[]) constrs in
    let lconstrs = List.flatten lconstrs in
      ...
      List.fold_left 
        (
          fun (acc,lin_obj) lconstr -> 
            let (x,_,_) = List.hd lconstr in 
            match lin_obj with
                        []                                      -> ((lconstr,zero)::acc,[])
                      | [(c,x')]::lin_obj when compare x' x = 0 -> ((lconstr,c)::acc,lin_obj)  
                      | [(_,_)]::_                             -> ((lconstr,zero)::acc,lin_obj)  
                      | _ -> assert false
        ) 
        ([],lin_obj) 
        lconstrs 
    ...
    let create_constr (lhs,rhs) =
      let lhs = DualLP.Sum (List.map (fun (_,(constr,_),c) -> DualLP.Mul(c,DualLP.Var(constr))) lhs) in
      let rhs = DualLP.Const rhs in
      DualLP.Eq(lhs,rhs)
    in
    List.rev_append (List.map create_constr lconstrs_and_lin_obj) slacks
  ...
  let dualize (lp : PrimalLP.lp) = 
    ...
    let obj = match obj with PrimalLP.Max obj -> obj | _ -> assert false in
    let obj = PrimalLP.normal_expr obj in
    let constrs = lp.PrimalLP.constrs in
    let constrs = List.map (fun constr -> (constr, PrimalLP.normal_constr constr)) constrs in 
    let is_non_trivial = function 
