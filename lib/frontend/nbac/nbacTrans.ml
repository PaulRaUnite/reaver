...
let default_strat_h = "aB;aB:b;pIF;pMHB;pQ;rT;aH:ws=5"
...
let parse ?(is_file=true) input =
  let prog = 
    if is_file && input<>"" then
      Parse.parse_file ~lexer:NbacLexer.lexBac ~parser:NbacParser.prog input
    else if not is_file && input<>"" then
      Parse.parse_lexbuf ~lexer:NbacLexer.lexBac ~parser:NbacParser.prog 
