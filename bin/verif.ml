(******************************************************************************)
(* verif *)
(* verification strategy engine - without max-strategy iteration *)
...
let get_domain_desc dom = 
 let (domstr,_) = ParseParams.parse_param dom in
  fst (List.assoc domstr domain_descs)
...
let str_to_strategy env str =
  ...
  List.map (fun s ->
    let p = try ParseParams.parse_param s 
      with Exit | Parse.Lex_error | Parsing.Parse_error -> raise (InvalidStrategy s)
    in
    match p with
